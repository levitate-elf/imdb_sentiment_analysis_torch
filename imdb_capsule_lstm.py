...
class Capsule(nn.Module):
    def __init__(self, num_hiddens, bidirectional, num_capsule=5, dim_capsule=5, routings=4, **kwargs):
        ...
        in_dim = num_hiddens * 2 if bidirectional else num_hiddens
        self.W = nn.Parameter(nn.init.xavier_normal_(torch.empty(in_dim, num_capsule * dim_capsule)))
    ...
    def forward(self, inputs):
        print(inputs.shape)
        print(self.W.shape)
        B, T, in_dim = inputs.shape
        u_hat_vecs = torch.matmul(inputs, self.W)
        print(u_hat_vecs.shape)
        u_hat_vecs = u_hat_vecs.view(B, T, self.num_capsule, self.dim_capsule) \
            .permute(0, 2, 1, 3).contiguous()
        with torch.no_grad():
            b = inputs.new_zeros((B, self.num_capsule, T))
        for i in range(self.routings):
            c = torch.softmax(b, dim=1)  # (batch_size,num_capsule,input_num_capsule)
...
class SentimentNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        encoding = capsule.flatten(start_dim=1)
