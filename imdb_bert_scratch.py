...
class BertScratch(BertPreTrainedModel):
    def __init__(self, config):
        ...
        p = config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None):
        ...
        pooled = outputs.pooler_output          # 等价于 outputs[1]
        logits = self.classifier(self.dropout(pooled))

        loss = None
        if labels is not None:                  # ← 只有有标签时才算 loss
